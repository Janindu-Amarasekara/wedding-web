...
function RSVPForm() {
  ...
  useEffect(() => {
    if (!recaptchaSiteKey) {
      console.warn(
        "reCAPTCHA Site Key not found. Set VITE_RECAPTCHA_SITE_KEY in your .env file"
      );
    ...
    console.log(
      "Loading reCAPTCHA v3 with site key:",
  ...
  const handleSubmit = async (e) => {
    ...
    if (recaptchaSiteKey) {
      if (!window.grecaptcha) {
        console.error(
          "reCAPTCHA not loaded. Check your site key and network connection."
        );
        setStatus({
          type: "error",
          message:
            "Security verification not loaded. Please refresh the page and try again.",
        });
        return;
      }
      ...
      if (recaptchaToken) {
          ...
          await supabase.functions.invoke("verify-recaptcha", {
            body: {
              ...
              rsvpData: {
                ...
                guests:
                  form.attending === "yes" ? parseInt(form.guests, 10) : 0,
        ...
        if (functionError) {
          ...
          setStatus({
            type: "error",
            message:
              functionData.error === "reCAPTCHA score too low"
          ...
          console.log(
            "RSVP submitted successfully with score:",
            functionData.score
          );
          setStatus({
            type: "success",
